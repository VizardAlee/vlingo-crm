...
export const guideTopics: GuideTopic[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      "On a phone or tablet, use the focused Products, Customer, and Pay steps. The cart count stays visible in the Pay step label, and desktop continues to show the full checkout workspace.",
      ...
      "Choose Walk-in, Existing, or New under Customer. Existing searches the organization's customer database by name, phone, email, or customer number and fills the verified details automatically, even when the customer's home branch differs from the sale branch. New requires a name and phone and can save a reusable Clients record; Walk-in creates no customer record.",
      "A sale linked to an existing or newly saved customer stores both the permanent customer ID and a sale-time contact snapshot. This supports customer purchase audits without allowing later profile edits to rewrite historical invoices.",
      "Set the unit selling price for each cart line. Use the retail or wholesale shortcut when configured, or enter an authorized adjusted price. Then apply any separate line discount and tax required; the server records the selected unit price and checks stock again before completing the sale.",
      "Choose the Invoice and receipt brand: Vlingo Systems or Kada Builders Mart. The choice is stored on the sale and applies to its invoice and every receipt, including receipts for later payments.",
      "Enter the amount received and payment method. Full, partial, and unpaid sales are supported.",
      "Complete the sale. The system deducts stock, records sale movements, creates a branch-coded VSL or KBM invoice number, and creates a separate numbered receipt whenever money is received—all in one transaction.",
      "Open Sales history to print the branded A4 invoice or receipt. Both designs include customer and location details, itemized prices, amount in words, and payment status. Vlingo documents use the Vlingo letterhead, configured Lotus Bank instructions, authorised signature, and official stamp. Kada documents use the supplied Kada green-and-orange identity and Kada-specific business wording without Vlingo bank or stamp details.",
      ...
      "Managing Directors, Super Admins, and Operations Managers acting within their authorized branches can use Adjust in Sales history to correct customer details, quantities, prices, discounts, tax, or notes. A reason is compulsory; the server recalculates the balance, posts stock differences as new movements, retains receipts, and records the before-and-after values in the audit log.",
      "Use Void instead of deleting a completed sale. Voiding retains the invoice and receipts with a VOID label, restores the sold stock, reverses linked finance payments, excludes the transaction from completed-sales reports, and records the reason and user permanently. Products cannot be swapped during an adjustment; void and re-enter that sale instead.",
  ...
  },
  {
    ...
    steps: [
      ...
      "For a transfer, the source is the active dashboard branch. A user with inventory transfer permission can choose any active organization branch as the destination even when they cannot normally open that branch; this does not grant access to view or manage the destination branch, and both branch ledgers show the transfer.",
  ...
  },
  {
    ...
    steps: [
      ...
      "On the Inventory tab, use the numbered period report: opening stock, stock received, POS sales with salesperson and selling price, internal transfers, closing reconciliation, summary, and calculation notes. Current stock, location balances, valuation, and the full movement ledger follow as appendices.",
      "Opening and closing quantities are reconstructed from the audited movement ledger and current system balances. The reconciliation is opening + receipts + transfers in + other stock in - sales - transfers out - other stock out.",
      "Use CSV for spreadsheet analysis or A4 PDF for the formal inventory report; both include the selected date, branch, and brand scope and work without requesting an AI summary.",
      "Inventory balances and value are current point-in-time figures. Historical sales prices come from completed POS records; a manual sale issue without a linked POS sale can affect quantity without supplying a selling price.",
    ],
  },
  {
  ...
  },
  {
    ...
    steps: [
      ...
      "Products are defined once for the organization. When Inventory receives or transfers an existing product into another branch, its destination balance automatically makes that same product visible in the destination branch's Products/Services catalogue and POS; users should not create a duplicate product for each branch.",
      "Authorized users can choose Edit beside an item in Inventory to change its catalog details. Users with inventory adjustment permission also see Adjust current stock on that edit page: choose the branch, add or remove the difference, enter the date and reason, then apply it as a separate audited ledger movement. The original opening record is never overwritten.",
...
- Point of Sale: branch-aware checkout, searchable existing customers, reusable new-customer profiles, walk-in sales, directly typed whole-number quantities with plus/minus shortcuts, editable unit prices with retail/wholesale presets, discounts, tax, full/partial/unpaid sales, stock deduction, sales history, later payments, privileged audited sale adjustments and void reversals, and selectable Vlingo Systems or Kada Builders Mart printable invoices and receipts.
...
- Point of Sale sells from the active branch's available balance. A user can type a full whole-number cart quantity, use plus/minus shortcuts, and set each line to its catalog retail price, configured wholesale price, or an adjusted unit price before checkout. The recorded sale preserves the chosen price and its difference from retail. Users can also choose Vlingo Systems or Kada Builders Mart for the sale documents. Checkout creates the sale, branch-coded VSL or KBM invoice number, optional numbered receipt and finance payment, inventory issues, and stock deductions atomically. It supports walk-in customers, discounts, tax, unpaid invoices, part payments, and a separate consistently branded receipt for every later payment.
- On phones and tablets, Point of Sale uses three focused steps: Products, Customer, and Pay. Product cards show available stock, prices, and the quantity already in the cart; large quantity controls and full-width navigation make checkout easier without changing the desktop workflow.
...
- Printable POS invoices and receipts use the brand selected at checkout and a fixed A4 document structure: customer/location metadata, itemized lines, totals, amount in words, and payment status. Vlingo invoices contain the configured Lotus Bank payment details and Vlingo receipts contain the approved authorised signature, official stamp, and current Lagos date. Kada documents use the supplied Kada Builders Mart identity and do not show Vlingo-specific bank, signature, stamp, or company wording.
- Products can be created individually or imported from CSV/XLS/XLSX through a column-mapping and validation preview. Catalog records support a regular selling price and an optional wholesale price. Inventory products require an existing active brand, use an automatically generated SKU, and may omit barcode/GTIN. During individual product creation, users with inventory.receive can optionally record existing initial stock as a ledger-backed opening-balance receipt at the selected branch; imports do not include quantity, and later procured stock enters through Inventory. Users with inventory.manageCatalog can create a missing brand from the product form. Admin-created active branches are the stock locations used for new inventory.
