...
export function columnsFor(moduleKey: ModuleKey): ColumnDef<Record<string, unknown>>[] {
  ...
  if (moduleKey === "deals") {
    return [
      ...
      { header: "Products/services", cell: ({ row }) => Array.isArray(row.original.quoteLines) ? `${row.original.quoteLines.length} quotation line${row.original.quoteLines.length === 1 ? "" : "s"}` : String(row.original.offeringName ?? "Not linked") },
      { header: "Value", cell: ({ row }) => formatCurrency(Number(row.original.agreedAmount ?? row.original.quoteTotal ?? row.original.offerAmount ?? row.original.depositAmount ?? row.original.reservationAmount ?? 0)) },
