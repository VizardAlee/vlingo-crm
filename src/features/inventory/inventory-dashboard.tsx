...
import type { InventoryBalance, InventoryBrand, InventoryComment, InventoryLocation, InventoryMovement, InventoryMovementType, Offering } from "@/types/crm";

type Tab = "overview" | "movements" | "comments" | "setup";
...
export function InventoryDashboard() {
  ...
  const [movement, setMovement] = useState({ movementType: "receipt" as InventoryMovementType, offeringId: "", quantity: 1, fromLocationId: "", toLocationId: "", externalReference: "", notes: "", occurredAt: new Date().toISOString().slice(0, 10) });
  ...
  const lowStock = itemTotals.filter((item) => item.reorderLevel !== undefined && item.quantity <= Number(item.reorderLevel)).length;
  ...
  async function submitComment(event: React.FormEvent) {
    ...
    setSaving("comment");
    try {
      await addInventoryComment({ ...commentForm, organizationId: activeOrganizationId, branchId: activeBranchId, userId: user.uid, userEmail: member?.email, userName: member?.displayName });
  ...
  function downloadReport() {
    const header = ["Brand", "SKU", "Item", "Location", "Quantity", "Reorder level", "Unit cost", "Stock value"];
    const rows = balances.map((balance) => {
      const item = items.find((entry) => entry.id === balance.offeringId);
      return [balance.brandName, balance.sku, balance.offeringName, balance.locationName, balance.quantityOnHand, item?.reorderLevel ?? "", item?.costPrice ?? 0, Number(balance.quantityOnHand) * Number(item?.costPrice ?? 0)];
    });
  ...
  return (
    <section className="grid min-w-0 gap-5">
      <div className="rounded-md bg-white p-4 shadow-sm md:flex md:items-end md:justify-between md:bg-transparent md:p-0 md:shadow-none">
        ...
        <div className="mt-4 flex gap-2 md:mt-0"><Button onClick={downloadReport} variant="outline"><Download className="h-4 w-4" />Export CSV</Button><Button onClick={load} variant="outline"><RefreshCw className="h-4 w-4" />Refresh</Button></div>
      </div>
      {error ? <ErrorState message={error} /> : null}
      <div className="flex gap-2 overflow-x-auto pb-1">{(["overview", "movements", "comments", ...(canSetup ? ["setup"] : [])] as Tab[]).map((value) => <Button key={value} onClick={() => setTab(value)} variant={tab === value ? "primary" : "outline"}>{titleCase(value)}</Button>)}</div>
      ...
      {tab === "overview" ? <>
        ...
        <Card><CardHeader><CardTitle>Stock by item</CardTitle></CardHeader><CardContent className="overflow-x-auto p-0"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-muted/70 text-xs uppercase text-muted-foreground"><tr><th className="px-4 py-3">Brand / item</th><th className="px-4 py-3">SKU</th><th className="px-4 py-3">On hand</th><th className="px-4 py-3">Reorder</th><th className="px-4 py-3">Value</th><th className="px-4 py-3">Status</th></tr></thead><tbody>{itemTotals.map((item) => { const low = item.reorderLevel !== undefined && item.quantity <= Number(item.reorderLevel); return <tr className="border-t" key={item.id}><td className="px-4 py-3"><div className="font-semibold">{item.name}</div><div className="text-xs text-muted-foreground">{item.brandName}</div></td><td className="px-4 py-3">{item.sku || "—"}</td><td className="px-4 py-3 font-semibold">{item.quantity}</td><td className="px-4 py-3">{item.reorderLevel ?? "—"}</td><td className="px-4 py-3">{formatCurrency(item.quantity * Number(item.costPrice ?? 0))}</td><td className="px-4 py-3"><Badge tone={low ? "warning" : "success"}>{low ? "Low stock" : "Healthy"}</Badge></td></tr>; })}{!itemTotals.length ? <tr><td className="px-4 py-8 text-center text-muted-foreground" colSpan={6}>No branded inventory items are available yet.</td></tr> : null}</tbody></table></CardContent></Card>
      ...
      {tab === "movements" ? <div className="grid gap-5 xl:grid-cols-[380px_1fr]">
        {canMoveStock ? <Card><CardHeader><CardTitle>Record stock movement</CardTitle></CardHeader><CardContent><form className="grid gap-3" onSubmit={submitMovement}><Field label="Movement type"><Select value={movement.movementType} onChange={(event) => setMovement((value) => ({ ...value, movementType: event.target.value as InventoryMovementType }))}>{movementTypes.map((type) => <option key={type} value={type}>{titleCase(type)}</option>)}</Select></Field><Field label="Item"><Select required value={movement.offeringId} onChange={(event) => setMovement((value) => ({ ...value, offeringId: event.target.value }))}><option value="">Select item</option>{items.map((item) => <option key={item.id} value={item.id}>{item.brandName} · {item.name}</option>)}</Select></Field><Field label="Quantity"><Input min={1} required type="number" value={movement.quantity} onChange={(event) => setMovement((value) => ({ ...value, quantity: Number(event.target.value) }))} /></Field>{needsFrom ? <Field label="Source location"><Select required value={movement.fromLocationId} onChange={(event) => setMovement((value) => ({ ...value, fromLocationId: event.target.value }))}><option value="">Select source</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</Select></Field> : null}{needsTo ? <Field label="Destination location"><Select required value={movement.toLocationId} onChange={(event) => setMovement((value) => ({ ...value, toLocationId: event.target.value }))}><option value="">Select destination</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</Select></Field> : null}<Field label="Movement date"><Input type="date" value={movement.occurredAt} onChange={(event) => setMovement((value) => ({ ...value, occurredAt: event.target.value }))} /></Field><Field label="Supplier, PO, job or sale reference"><Input value={movement.externalReference} onChange={(event) => setMovement((value) => ({ ...value, externalReference: event.target.value }))} /></Field><Field label="Notes"><Textarea value={movement.notes} onChange={(event) => setMovement((value) => ({ ...value, notes: event.target.value }))} /></Field><Button disabled={saving === "movement"} type="submit"><PackageCheck className="h-4 w-4" />{saving === "movement" ? "Recording" : "Record movement"}</Button></form></CardContent></Card> : null}
